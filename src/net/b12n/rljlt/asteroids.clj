...
(def ^:private SIZES {3 {:r 40.0 :score 20}
                      2 {:r 22.0 :score 50}
                      1 {:r 11.0 :score 100}})
...
(defn- rand-unit []                       ; a random double in [-1, 1)
  ...
  (if (:over? s)
    ...
    (let [ship  (:ship s)
          ...
          bullets (->> (into (:bullets s) fired)
                       (map (fn [b] (assoc b :x (wrap (+ (:x b) (:vx b)) W)
                                           :y (wrap (+ (:y b) (:vy b)) H)
                                           :life (dec (:life b)))))
                       (filterv (fn [b] (pos? (:life b)))))
          asteroids (mapv (fn [a] (assoc a :x (wrap (+ (:x a) (:vx a)) W)
                                         :y (wrap (+ (:y a) (:vy a)) H)
...
;; --- draw --------------------------------------------------------------------
(defn- draw-asteroid [a]
  (let [{:keys [x y angle verts]} a
        ca  (Math/cos angle) sa (Math/sin angle)
        pts (mapv (fn [[dx dy]] [(+ x (- (* dx ca) (* dy sa)))
                                 (+ y (+ (* dx sa) (* dy ca)))]) verts)
        n   (count pts)]
    (doseq [i (range n)]
      (let [[ax ay] (nth pts i)
...
(defn- draw-ship [ship]
  (let [{:keys [x y angle]} ship
        pt (fn [da] [(+ x (* SHIP-R (Math/cos (+ angle da))))
